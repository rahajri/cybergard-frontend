...
import {
  ...
  ChevronDown, ChevronUp, GripVertical, X, Search, Folder
} from 'lucide-react';
...
interface Questionnaire {
  ...
  source_type: 'framework' | 'control_points' | 'manual';
  framework_id?: string;
  source_ref?: string;
...
export default function QuestionnaireEditorPage({
  ...
  const loadQuestionnaire = async () => {
    ...
    try {
      ...
      const questionnaireResponse = await fetch(`${API_BASE}/api/v1/questionnaires/${questionnaireId}`);
      ...
      const questionsResponse = await fetch(`${API_BASE}/api/v1/questionnaires/${questionnaireId}/questions`);
  ...
  const handleSaveQuestionnaire = async () => {
    ...
    setSaving(true);
    try {
      // 1. Sauvegarder les métadonnées du questionnaire
      const response = await fetch(`${API_BASE}/api/v1/questionnaires/${questionnaireId}`, {
        method: 'PUT',
      ...
      const deletePromises = Array.from(questionsToDelete).map(async (questionId) => {
        ...
        const deleteResponse = await fetch(
          `${API_BASE}/api/v1/questionnaires/${questionnaireId}/questions/${questionId}`,
          { method: 'DELETE' }
        );
  ...
  const handleSaveQuestion = async (questionId: string, questionData: Partial<Question>) => {
    ...
    try {
      ...
      const endpoint = isNew
        ? `${API_BASE}/api/v1/questionnaires/${questionnaireId}/questions`
        : `${API_BASE}/api/v1/questionnaires/${questionnaireId}/questions/${questionId}`;
  ...
  const confirmDeleteQuestion = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/v1/questionnaires/${questionnaireId}/questions/${deleteModal.questionId}`,
        { method: 'DELETE' }
      );
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 to-slate-100 questionnaire-editor-namespace">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="questionnaire-editor-page" style={{ backgroundColor: 'transparent', color: '#1f2937' }}>
            <QuestionnaireTitleBar
              ...
              onBack={() => router.push('/admin/questionnaires')}
            />
          </div>
        </div>
      </header>
      ...
      <main className="flex-1">
        ...
        <div className="editor-main-content" style={{ color: '#1f2937' }}>
          ...
          <div className="questions-by-domain">
            {domainGroups.map((group, groupIndex) => (
              <DomainQuestionGroup
                ...
                onEdit={(id) => setEditorState(prev => ({ ...prev, editingId: id }))}
          ...
          {questions.length === 0 && (
            <EmptyQuestionsState onAddFirst={handleAddQuestion} />
          )}
        </div>
        ...
        {editorState.hasUnsavedChanges && !editorState.editingId && (
          <FloatingActions
            hasUnsavedChanges={editorState.hasUnsavedChanges}
            saving={saving}
            onSave={handleSaveQuestionnaire}
  ...
  <div className="metadata-card">
    <div className="metadata-grid">
      ...
      <div className="metadata-item">
        ...
        <span className="value">{labelFrom(SOURCE_LABELS, questionnaire.source_type)}</span>
      </div>
      <div className="metadata-item">
        <Calendar size={16} />
        <span className="label">Créé le</span>
        <span className="value">{new Date(questionnaire.created_at).toLocaleDateString('fr-FR')}</span>
      </div>
      {questionnaire.created_by && (
        <div className="metadata-item">
          <User size={16} />
          <span className="label">Créé par</span>
          <span className="value">{questionnaire.created_by}</span>
        </div>
      )}
    </div>
  </div>
...
const EditorToolbar = ({
  ...
  <div className="editor-toolbar">
    ...
    <div className="toolbar-actions">
      {selectedCount && selectedCount > 0 && onDeleteSelection && (
      ...
      <button onClick={onRegenerateAI} className="regenerate-button">
        <Sparkles size={16} />
        Régénérer via IA
      </button>
...
const QuestionEditor = ({
  ...
  const loadFrameworks = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/frameworks?limit=100`);
      if (response.ok) {
  ...
  const loadDomains = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/frameworks/${selectedFrameworkId}/domains`);
      if (response.ok) {
        const data = await response.json();
  ...
  const loadRequirements = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/v1/frameworks/${selectedFrameworkId}/requirements?section_id=${selectedDomainId}`
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleRequirementChange = async (requirementId: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/requirements/${requirementId}/control-points`);
      if (response.ok) {
...
const NotFoundState = () => (
  <div className="not-found-state">
    ...
    <Link href="/questionnaires" className="back-link">
      Retour aux questionnaires
    </Link>
  </div>
);

const EmptyQuestionsState = ({ onAddFirst }: { onAddFirst: () => void }) => (
