...
import { AlertTriangle, X, Trash2, Loader2, Database } from 'lucide-react';
...
export default function DeleteOrganizationModal({
  ...
  const elements: Array<{ count: number; label: string; color: 'blue' | 'green' | 'yellow' | 'red' }> = [
    { count: userCount || 0, label: `Utilisateur${userCount > 1 ? 's' : ''}`, color: 'blue' },
    { count: userCount || 0, label: `Rôle${userCount > 1 ? 's' : ''} attribué${userCount > 1 ? 's' : ''}`, color: 'green' },
    { count: 1, label: 'Organisation', color: 'yellow' },
  ];

  if (tenantId) {
    elements.push({ count: 1, label: 'Tenant (si non partagé)', color: 'red' });
  }
