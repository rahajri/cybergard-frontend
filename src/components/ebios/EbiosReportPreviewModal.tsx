...
interface EbiosReportPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  reportId: string;
  reportTitle: string;
...
export default function EbiosReportPreviewModal({
  isOpen,
  onClose,
  reportId,
  reportTitle,
  fileName
}: EbiosReportPreviewModalProps) {
  ...
  const loadHtmlPreview = async () => {
    try {
      ...
      // Charger l'aperçu HTML depuis l'endpoint EBIOS
      const response = await fetchWithAuth(
        `${API_BASE}/api/v1/risk/reports/${reportId}/preview-html`
      );
  ...
  const handleDownloadHtml = () => {
    if (htmlContent) {
      ...
      a.href = url;
      a.download = fileName?.replace('.pdf', '.html') || `rapport_ebios_${reportId}.html`;
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      // Télécharger le vrai PDF généré sur le serveur (endpoint EBIOS)
      const response = await fetchWithAuth(
        `${API_BASE}/api/v1/risk/reports/${reportId}/download`
      );
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = fileName || `rapport_ebios_${reportId}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-red-50 to-orange-50">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors disabled:opacity-50"
        ...
        <div className="flex-1 overflow-auto bg-gray-100">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[500px]">
              ...
              <p className="text-gray-600">Chargement du rapport EBIOS RM...</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[500px]">
