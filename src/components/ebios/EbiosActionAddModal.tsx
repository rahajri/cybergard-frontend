...
import { X, Save, AlertCircle, Calendar, User, Target, FileText, Shield, Building2, TrendingDown, Bookmark, Zap, Clock, CheckCircle, Users, Plus } from 'lucide-react';

export interface EbiosActionCreateData {
  code_action?: string;
  titre: string;
  description: string;
  categorie: string;
...
interface ScopeEntity {
  id: string;
  name: string;
  stakeholder_type: string; // 'internal' | 'external'
...
interface EbiosActionAddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (newAction: EbiosActionCreateData) => Promise<void>;
  studyId: string;
}
...
const initialFormData: EbiosActionCreateData = {
  code_action: '',
  titre: '',
  description: '',
  categorie: '',
...
export function EbiosActionAddModal({ isOpen, onClose, onSave, studyId }: EbiosActionAddModalProps) {
  const [formData, setFormData] = useState<EbiosActionCreateData>(initialFormData);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header - Gradient Rouge pour module EBIOS */}
        <div className="sticky top-0 bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-semibold">Ajouter une action EBIOS</h3>
              <p className="text-red-100 text-sm">Création manuelle</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* Code Action (optionnel) */}
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center">
              <FileText className="w-4 h-4 mr-2 text-gray-500" />
              Code de l'action (optionnel)
            </label>
            <input
              type="text"
              value={formData.code_action || ''}
              onChange={(e) => setFormData({ ...formData, code_action: e.target.value })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
              placeholder="Laissez vide pour générer automatiquement (ACT_MAN_XXX)"
            />
            <p className="text-xs text-gray-500 mt-1">
              Si non renseigné, un code sera généré automatiquement
            </p>
          </div>
